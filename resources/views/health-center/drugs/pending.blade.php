        ...
        <div class="col-md-4">
            <div class="card bg-info text-white">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h4 class="card-title">{{ $pendingDrugs->where('created_at', '>=', now()->subDays(7))->count() }}</h4>
        ...
        <div class="col-md-4">
            <div class="card bg-secondary text-white">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h4 class="card-title">{{ $pendingDrugs->where('created_at', '<', now()->subDays(30))->count() }}</h4>
                        ...
                        <tbody>
                            @foreach($pendingDrugs as $drug)
                                @php
                                    ...
                                    $daysPending = $drug->created_at->diffInDays(now());
                                @endphp
                                <tr>
                                    <td>
                                    ...
                                    <td>
                                        @if($daysPending < 7)
                                            <span class="badge bg-success">{{ $daysPending }} يوم</span>
                                        @elseif($daysPending < 30)
                                            <span class="badge bg-warning">{{ $daysPending }} يوم</span>
                                        @else
                                            <span class="badge bg-danger">{{ $daysPending }} يوم</span>
                                        @endif
                                    </td>
                                    <td>
